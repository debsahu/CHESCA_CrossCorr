...
% maximum residue number of protein of interest
prompt = 'Maximum number of residues? [268]: ';
maxres = input(prompt);
if isempty(maxres)
    maxres = 268;
end

% cut off for p value to pick statistically significat R
prompt = 'What p-cutoff value do you want to use? [0.01]: ';
pcuttoff = input(prompt);
if isempty(pcuttoff)
    pcuttoff = 0.01;
end
...
while 1
    prompt = 'Name of Output File (adds "KS_" to this input): ';
    OutMatrixFileName = input(prompt,'s');
    if ~isempty(OutMatrixFileName)
        OutMatrixFileName = ['KS_' OutMatrixFileName];
        break
    end
end
